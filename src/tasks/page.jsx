import Modal from '/src/tasks/modal/modal.jsx';
...
function Tasks() {
    ...
    const [tasks, setTasks] = useState([
        { id: 1, name: 'new task', items: 0 },
        { id: 2, name: 'even newer tasks', items: 0 },
    ])
            ...
            <div className="task-container">
                {tasks.map(task => (
                    <div className="task" key={task.id}>
                        <p className="task-number">{task.name}</p>
                        <p className="item">{task.items} Items</p>
                        <button className="edit-button">edit list</button>
                        <button className="delete-button" onClick={() => handleDeleteTask(task.id)}>delete list</button>
                    </div>
                ))}
            </div>
